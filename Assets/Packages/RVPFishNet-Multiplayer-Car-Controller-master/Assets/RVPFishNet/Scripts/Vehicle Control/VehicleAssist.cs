...
namespace RVP
{
    ...
    public class VehicleAssist : MonoBehaviour, IVehicleComponent
    {
        ...
        {
            writer.WriteSingle(_rb.angularDamping);
        }

        void IVehicleComponent.SetFullState(Reader reader)
        {
            _rb.angularDamping = reader.ReadSingle();
        }
        ...
        void IVehicleComponent.Simulate()
        {
            ...
            else
            {
                if (angularDragOnJump)
                {
                    ...
                    _rb.angularDamping = angDragTime > 0 && _vp.upDot > 0.5 ? 10 : _initialAngularDrag;
                }
            }
        ...
        private void ApplySpinAssist()
        {
            ...
            float rightVelDot = Vector3.Dot(transform.right, _rb.linearVelocity.normalized);
        ...
        {
            float pushFactor = (_vp.accelAxisIsBrake ? _vp.accelInput : _vp.accelInput - _vp.brakeInput) * Mathf.Abs(_vp.localVelocity.x) * driftPush * _groundedFactor * (1 - Mathf.Abs(Vector3.Dot(_vp.forwardDir, _rb.linearVelocity.normalized)));
